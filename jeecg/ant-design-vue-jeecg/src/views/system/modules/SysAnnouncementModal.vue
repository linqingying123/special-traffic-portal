  ...
  <a-modal
    ...
    <a-spin :spinning="confirmLoading">
      <a-form-model ref="form" :model="model" :rules="validatorRules">
        <a-form-model-item :labelCol="labelCol" :wrapperCol="wrapperCol" label="消息类型" prop="msgCategory">
          <a-radio-group v-model="model.msgCategory" :disabled="disableSubmit">
            <a-radio value="1">通知公告</a-radio>
            <a-radio value="2">系统消息</a-radio>
          </a-radio-group>
        </a-form-model-item>
        <a-form-model-item :labelCol="labelCol" :wrapperCol="wrapperCol" label="标题" prop="titile">
          <a-input v-model="model.titile" :readOnly="disableSubmit" placeholder="请输入标题"/>
        </a-form-model-item>
        <a-form-model-item :labelCol="labelCol" :wrapperCol="wrapperCol" label="摘要" prop="msgAbstract">
          <a-textarea v-model="model.msgAbstract"  placeholder="请输入摘要" />
        </a-form-model-item>
        <a-form-model-item :labelCol="labelCol" :wrapperCol="wrapperCol" class="endTime" label="截至日期" prop="endTime">
          <j-date v-model="model.endTime" :getCalendarContainer="node => node.parentNode" dateFormat="YYYY-MM-DD HH:mm:ss" placeholder="请选择结束时间" showTime style="width: 100%" />
        </a-form-model-item>
        <a-form-model-item :labelCol="labelCol" :wrapperCol="wrapperCol" label="接收用户" prop="msgType">
        ...
        <a-form-model-item v-if="userType" :labelCol="labelCol" :wrapperCol="wrapperCol" label="指定用户">
          <j-select-multi-user v-model="userIds" :returnKeys="returnKeys" :trigger-change="true" placeholder="请选择指定用户"></j-select-multi-user>
        </a-form-model-item>
        <a-form-model-item :labelCol="labelCol" :wrapperCol="wrapperCol" label="优先级" >
          <a-radio-group v-model="model.priority" :disabled="disableSubmit" placeholder="请选择优先级">
            <a-radio value="L">低</a-radio>
            <a-radio value="M">中</a-radio>
            <a-radio value="H">高</a-radio>
          </a-radio-group>
        </a-form-model-item>
        <a-form-model-item :labelCol="labelColX1" :wrapperCol="wrapperColX1" class="j-field-content" label="内容">
