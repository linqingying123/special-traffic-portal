<template>
  <a-card>
    <draggable v-model="dataSource" :options="{animation: 300}" style="display: inline-block" @end="end">
      <template v-for="(data,index) in dataSource">
        <div :key="index" style="float: left;width:150px;height:150px;margin-right: 10px;margin: 0 8px 8px 0;">
          <div style="width: 100%;height: 100%;position: relative;padding: 8px;border: 1px solid #d9d9d9;border-radius: 4px;">
            <img :src="data.filePath" preview="index" style="width: 100%;">
          </div>
        </div>
      </template>
      <a-button style="margin-top: 115px" type="primary" @click="sureChange">确定</a-button>
    </draggable>
    <br/>
    <a-row>
      <a-col :span="12">
