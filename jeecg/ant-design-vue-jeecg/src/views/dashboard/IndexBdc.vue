  ...
  <div class="page-header-index-wide">
    <a-row :gutter="24">
      <a-col :md="12" :sm="24" :style="{ marginBottom: '24px' }" :xl="6">
        <chart-card :loading="loading" :total="cardCount.sll | NumberFormat" title="受理量">
      ...
      <a-col :md="12" :sm="24" :style="{ marginBottom: '24px' }" :xl="6">
        <chart-card :loading="loading" :total="cardCount.bjl | NumberFormat" title="办结量">
          <a-tooltip slot="action" title="指标说明">
            <a-icon type="info-circle-o" />
          </a-tooltip>
          <div>
      ...
      <a-col :md="12" :sm="24" :style="{ marginBottom: '24px' }" :xl="6">
        <chart-card :loading="loading" :total="cardCount.isll | NumberFormat" title="用户受理量">
          <a-tooltip slot="action" title="指标说明">
            <a-icon type="info-circle-o" />
          </a-tooltip>
          <div>
      ...
      <a-col :md="12" :sm="24" :style="{ marginBottom: '24px' }" :xl="6">
        <chart-card :loading="loading" :total="cardCount.ibjl | NumberFormat" title="用户办结量">
          <a-tooltip slot="action" title="指标说明">
            <a-icon type="info-circle-o" />
          </a-tooltip>
          <div>
    ...
    <a-card :body-style="{padding: '0'}" :bordered="false" :loading="loading">
      <div class="salesCard">
        <a-tabs :tab-bar-style="{marginBottom: '24px', paddingLeft: '16px'}" default-active-key="1" size="large">
          ...
          <a-tab-pane key="1" loading="true" tab="受理监管">
            <a-row>
              ...
              <a-col :lg="12" :md="12" :sm="24" :xl="8" :xs="24">
                ...
                <a-card :body-style="{padding: 0}" :bordered="false" style="margin-bottom: 24px" title="快速开始 / 便捷导航">
                  <div class="item-group">
                    <a-row>
                      <a-col v-for="(item,index) in registerTypeList" :key=" 'registerType'+index " :class="'more-btn'" :span="12">
                        <a-button ghost size="small" style="margin-bottom:10px" type="primary" @click="goPage(index)">{{ item.text }}</a-button>
                      </a-col>
                    </a-row>
                  </div>
                </a-card>
          ...
          <a-tab-pane key="2" tab="交互监管">
            <a-row>
              <a-col :lg="12" :md="12" :sm="24" :xl="16" :xs="24">
                <bar-multid :fields="jhjgFields" :sourceData="jhjgData" title="平台与部门交互量统计"></bar-multid>
              </a-col>
              <a-col :lg="12" :md="12" :sm="24" :xl="8" :xs="24">
          ...
          <a-tab-pane key="4" tab="存储监管">
            <a-row>
              <a-col :lg="12" :md="12" :sm="24" :xl="16" :xs="24">
                <a-row>
                  <template v-if="diskInfo && diskInfo.length>0">
                    <a-col v-for="(item,index) in diskInfo" :key=" 'diskInfo'+index " :span="12">
                      <dash-chart-demo :datasource="item.restPPT" :title="item.name"></dash-chart-demo>
                    </a-col>
                  </template>
                </a-row>
              </a-col>
              ...
              <a-col :lg="12" :md="12" :sm="24" :xl="8" :xs="24">
                ...
                <a-card :body-style="{padding: 0}" :bordered="false" style="margin-bottom: 24px" title="快速开始 / 便捷导航">
                  <div class="item-group">
                    <a-row>
                      <a-col v-for="(item,index) in registerTypeList" :key=" 'registerType'+index " :class="'more-btn'" :span="10">
                        <a-button ghost size="small" style="margin-bottom:10px" type="primary" @click="goPage(index)">{{ item.text }}</a-button>
                      </a-col>
                    </a-row>
                  </div>
    ...
    <a-row :gutter="12">
      <a-card :bordered="false" :class="{ 'anty-list-cust':true }" :loading="loading" :style="{ marginTop: '24px' }">

        <a-tabs v-model="indexBottomTab" :tab-bar-style="{marginBottom: '24px', paddingLeft: '16px'}" size="large">
          <div slot="tabBarExtraContent" class="extra-wrapper">
          ...
          <a-tab-pane key="1" loading="true" tab="业务流程限时监管">
            ...
            <a-table :columns="columns" :dataSource="dataSource1" :pagination="ipagination1" rowKey="id" size="default" @change="tableChange1">
              <template slot="flowRate" slot-scope="text, record, index">
                <a-progress :format="getPercentFormat" :percent="getFlowRateNumber(record.flowRate)" :strokeColor="getPercentColor(record.flowRate)" style="width:80px" />
              </template>
            </a-table>
          </a-tab-pane>

          <a-tab-pane key="2" loading="true" tab="业务节点限时监管">
            <a-table :columns="columns2" :dataSource="dataSource2" :pagination="ipagination2" rowKey="id" size="default" @change="tableChange2">
              <template slot="flowRate" slot-scope="text, record, index">
