...
<template>
  <a-spin :spinning="loading">
    ...
    <a-row type="flex">
      <a-col>
        <slot :target="getVM()" name="buttonBefore"/>
      </a-col>
      <a-col>
        <!-- 操作按钮 -->
        <div v-if="actionButton" class="action-button">
          <a-button-group v-if="buttonPermission('add')">
            <a-button :disabled="disabled" icon="plus" type="primary" @click="handleClickAdd">新增</a-button>
            <a-popover v-if="addButtonSettings" overlayClassName="j-add-btn-settings" placement="right">
              <a-row slot="title">
                <a-col :span="12">选项</a-col>
                <a-col :span="12" style="text-align: right;">
                  <a-tooltip title="保存为默认值">
                    <a-button icon="save" size="small" style="position: relative;left:4px;" type="link" @click="onAddButtonSettingsSave"/>
                  </a-tooltip>
                </a-col>
              </a-row>
              <template slot="content">
                <a-form-model :labelCol="{span:8}" :wrapperCol="{span:16}" layout="horizontal">
                  <a-form-model-item label="添加行数">
                    <a-input-number v-model="settings.addRowNum" :min="1"/>
                  </a-form-model-item>
                  <a-form-model-item label="添加位置">
                    <a-input-number v-model="settings.addIndex" :max="rows.length" :min="0"/>
          ...
          <template v-if="selectedRowIds.length>0">
            <a-popconfirm
              ...
              <a-button v-if="buttonPermission('batch_delete')" :disabled="disabled" icon="minus" type="primary">删除</a-button>
              <span class="gap"></span>
            </a-popconfirm>
      ...
      <a-col>
        <slot :target="getVM()" name="buttonAfter"/>
      </a-col>
    </a-row>

    <slot :target="getVM()" name="actionButtonAfter"/>

    <div :id="`${caseId}inputTable`" class="input-table">
      <!-- 渲染表头 -->
      <div ref="thead" class="thead">
        <div :style="{width: this.realTrWidth}" class="tr">
          <!-- 左侧固定td  -->
          <div v-if="dragSort" :style="style.tdLeft" class="td td-ds">
            <span></span>
          </div>
          <div v-if="rowSelection" :style="style.tdLeft" class="td td-cb">
            <!--:indeterminate="true"-->
            <a-checkbox
          ...
          <div v-if="rowNumber" :style="style.tdLeft" class="td td-num">
            <span>#</span>
          </div>
          <!-- 右侧动态生成td -->
      ...
      <div ref="scrollView" :style="{'max-height':maxHeight+'px'}" class="scroll-view">


        <!-- 渲染主体 body -->
        <div :id="`${caseId}tbody`" :style="tbodyStyle" class="tbody">
          <!-- 扩展高度 -->
          <div :style="`height:${getExpandHeight}px; z-index:${loading?'11':'9'};`" class="tr-expand"></div>
          <!-- 无数据时显示 -->
          ...
          >
            ...
            <template v-for="(row,rowIndex) in rows">
              ...
              >
                ...
                <div v-if="dragSort" :style="style.tdLeft" class="td td-ds" @dblclick="_handleRowInsertDown(rowIndex)" >
                  <a-dropdown :getPopupContainer="getParentContainer" :trigger="['click']">
                ...
                <div
                  ...
                  <template v-for="(id,i) in [`${col.key}${row.id}`]">
                    ...
                    <!-- native input -->
                    <label v-if="col.type === formTypes.input || col.type === formTypes.inputNumber" :key="i">
                        ...
                        >
                          ...
                          <template slot="addonBefore" style="width: 30px">
                            ...
                            <a-tooltip v-else-if="file.status==='done'" title="上传完成">
                              <a-icon style="color:#00DB00;" type="check-circle"/>
                            </a-tooltip>
                          ...
                          <template v-if="col.allowDownload!==false || col.allowRemove!==false" slot="addonAfter" style="width: 30px">
                            <a-dropdown :getPopupContainer="getParentContainer" :trigger="['click']" placement="bottomRight">
                              <a-tooltip :getPopupContainer="getParentContainer" title="操作">
                                <a-icon
                    ...
                    <div v-else-if="col.type === formTypes.file" :key="i">
                      <template v-for="(file,fileKey) of [(uploadValues[id]||{})]" v-if="hasUploadValue(id)">
                        <div :key="fileKey" style="position: relative;">
                          <a-tooltip v-if="file.status==='uploading'" :title="`上传中(${Math.floor(file.percent)}%)`">
                            <a-icon style="color:red;" type="loading"/>
                          ...
                          <template style="width: 30px">
                            <a-dropdown :getPopupContainer="getParentContainer" :trigger="['click']" placement="bottomRight" style="margin-left: 10px;">
                              <a-tooltip :getPopupContainer="getParentContainer" title="操作">
                                <a-icon v-if="file.status!=='uploading'" style="cursor: pointer;" type="setting"/>
                              </a-tooltip>
                      ...
                      <template v-for="(file,fileKey) of [(uploadValues[id]||{})]" v-if="hasUploadValue(id)">
                        <div :key="fileKey" style="position: relative;">
                          ...
                          <template v-else-if="uploadValues[id]['path']">
                            <img :src="getCellImageView(id)" alt="无图片" class="j-editable-image" @click="handleMoreOperation(id,'img',col)"/>
                          </template>
                    ...
                    <!-- else (normal) -->
                    <span v-else :key="i" :title="inputValues[rowIndex][col.key]" class="comp-normal" v-bind="buildProps(row,col)">{{ inputValues[rowIndex][col.key] }}</span>
                  </template>
                </div>
              </div>
              <!-- -- tr end -- -->
      ...
      <j-file-pop ref="filePop" :number="number" @ok="handleFileSuccess"></j-file-pop>
    </div>
  </a-spin>
</template>
