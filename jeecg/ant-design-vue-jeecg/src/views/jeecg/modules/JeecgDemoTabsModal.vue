          ...
          <a-row :gutter="16" class="form-row">
            ...
            <a-col :lg="8">
              <a-form-item label="执行人">
                <a-select v-decorator="['task.executor',{rules: [{ required: true, message: '请选择执行人'}]}  ]" placeholder="请选择执行人">
          ...
          <a-row :gutter="16" class="form-row">
            <a-col :lg="8">
              <a-form-item label="责任人">
                <a-select v-decorator="['task.manager',  {rules: [{ required: true, message: '请选择责任人'}]} ]" placeholder="请选择责任人">
            ...
            <a-col :lg="8">
              <a-form-item label="提醒时间">
                <a-time-picker v-decorator="['task.time', {rules: [{ required: true, message: '请选择提醒时间'}]} ]" style="width: 100%"/>
              </a-form-item>
            </a-col>
            <a-col :lg="8">
              <a-form-item
                label="任务类型">
                <a-select v-decorator="['task.type', {rules: [{ required: true, message: '请选择任务类型'}]} ]" placeholder="请选择任务类型">
          ...
          <a-tab-pane key="1" tab="Tab 1">
            ...
            <a-table :columns="columns" :dataSource="data" :pagination="false" size="middle">
              <template v-for="(col, i) in ['name', 'workId', 'department']" :slot="col" slot-scope="text, record, index">
                <a-tooltip  :defaultVisible="false" :overlayStyle="{ color: 'red' }" title="必填项">
                  <a-input v-if="record.editable" :key="col" :placeholder="columns[i].title"  :value="text" style="margin: -5px 0" @change="e => handlerRowChange(e.target.value, record.key, col)"/>
            ...
            <a-button icon="plus" style="width: 100%; margin-top: 16px; margin-bottom: 8px" type="dashed" @click="newRow">新增成员</a-button>
          </a-tab-pane>
          <a-tab-pane key="2" forceRender tab="Tab 2">
