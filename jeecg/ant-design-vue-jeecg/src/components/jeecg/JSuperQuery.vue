...
<div class="j-super-query-box">
  ...
  <slot :isActive="superQueryFlag" :isMobile="izMobile" :open="handleOpen" :reset="handleReset" name="button">
    <a-tooltip v-if="superQueryFlag" :mouseLeaveDelay="0.2" v-bind="tooltipProps">
      ...
      <a-button-group>
        <a-button type="primary" @click="handleOpen">
          <a-icon spin theme="twoTone" type="appstore"/>
          <span>高级查询</span>
        </a-button>
        <a-button v-if="izMobile" icon="delete" type="primary" @click="handleReset"/>
      </a-button-group>
    </a-tooltip>
    <a-button v-else icon="filter" type="primary" @click="handleOpen">高级查询</a-button>
  </slot>
  ...
  >
    ...
    <a-spin :spinning="loading">
      <a-row>
        <a-col :md="24-5" :sm="24">
          ...
          <a-form v-else layout="inline">
            ...
            <a-row style="margin-bottom: 12px;">
              <a-col :md="12" :xs="24">
                <a-form-item :labelCol="{md: 6,xs:24}" :wrapperCol="{md: 18,xs:24}" label="过滤条件匹配" style="width: 100%;">
            ...
            <a-row v-for="(item, index) in queryParamsModel" :key="index" :gutter="16" style="margin-bottom:10px" type="flex">
              ...
              <a-col :md="4" :xs="24" style="margin-bottom: 12px;">
                <a-select :getPopupContainer="node=>node.parentNode" :value="item.rule" placeholder="匹配规则" @change="handleRuleChange(item,$event)">
              ...
              <a-col :md="8" :xs="24" style="margin-bottom: 12px;">
                ...
                <a-select
                  ...
                  placeholder="请选择"
                />
                <j-area-linkage v-else-if="item.type==='area-linkage' || item.type==='pca'" v-model="item.val" style="width: 100%"/>
                <j-date v-else-if=" item.type=='date' " v-model="item.val" placeholder="请选择日期" style="width: 100%"></j-date>
                <j-date v-else-if=" item.type=='datetime' " v-model="item.val" :show-time="true" date-format="YYYY-MM-DD HH:mm:ss" placeholder="请选择时间" style="width: 100%"></j-date>
                ...
                <a-input-number v-else-if=" item.type=='int'||item.type=='number' " v-model="item.val" placeholder="请输入数值" style="width: 100%"/>
                <a-select v-else-if="item.type=='switch'" v-model="item.val" placeholder="请选择">
        ...
        <a-col :md="5" :sm="24">
          ...
          <a-card :bordered="true" class="j-super-query-history-card">
            <div slot="title">
              保存的查询
            </div>
    ...
    <a-modal :visible="prompt.visible" title="请输入保存的名称" @cancel="prompt.visible=false" @ok="handlePromptOk">
