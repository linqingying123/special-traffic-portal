<template>
  <a-menu v-show="visible" :selectedKeys="selectedKeys" :style="style" class="contextmenu" @click="handleClick">
    <a-menu-item v-for="item in itemList" :key="item.key">
      <a-icon v-if="item.icon" :type="item.icon" role="menuitemicon" />{{ item.text }}
    </a-menu-item>
  </a-menu>
</template>
