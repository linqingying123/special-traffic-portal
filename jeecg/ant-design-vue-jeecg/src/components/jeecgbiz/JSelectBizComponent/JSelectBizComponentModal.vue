<template>
  <j-modal
    ...
    <a-row :gutter="18">
      <a-col :span="16">
        <!-- 查询区域 -->
        <a-form class="j-inline-form" layout="inline">
          <!-- 固定条件 -->
          <a-form-item :label="(queryParamText||name)">
            <a-input v-model="queryParam[queryParamCode||valueKey]" :placeholder="'请输入' + (queryParamText||name)" @pressEnter="searchQuery"/>
          </a-form-item>
          <!-- 动态生成的查询条件 -->
          <j-select-biz-query-item v-if="queryConfig.length>0" v-show="showMoreQueryItems" :queryConfig="queryConfig" :queryParam="queryParam" @pressEnter="searchQuery"/>
          <!-- 按钮 -->
      ...
      <a-col :span="8">
        <a-card :body-style="{padding:0}" :bordered="false" :head-style="{padding:0}" :title="'已选' + name">

          <a-table :rowKey="rowKey" bordered size="middle" v-bind="selectedTable">
