  ...
  <a-card :bordered="false">
    <a-spin :spinning="loading">
      <a-form-model ref="form" :model="model" :rules="rules">
        <a-tabs>
          <a-tab-pane key="1" tab="消息选项">
            <a-form-model-item :labelCol="labelCol" :wrapperCol="wrapperCol" label="测试APP" prop="app">
              <a-select v-model="model.app" :options="appOptions" placeholder="请选择测试APP"/>
            </a-form-model-item>
            <a-form-model-item :labelCol="labelCol" :wrapperCol="wrapperCol" label="发送给所有人" prop="sendAll">
              <a-switch v-model="model.sendAll" checkedChildren="是" unCheckedChildren="否" @change="onSendAllChange"/>
            </a-form-model-item>
            <a-form-model-item :labelCol="labelCol" :wrapperCol="wrapperCol" label="接收人" prop="receiver">
              <j-select-multi-user v-model="model.receiver" :disabled="model.sendAll" placeholder="请选择接收人"/>
            </a-form-model-item>
            <a-form-model-item :labelCol="labelCol" :wrapperCol="wrapperCol" label="消息内容" prop="content">
              <a-textarea v-model="model.content" :rows="5" placeholder="请输入消息内容"/>
            </a-form-model-item>
            <div style="text-align: center;">
              <a-button size="large" style="width: 120px;" type="primary" @click="onSend">发送</a-button>
            </div>
          </a-tab-pane>
        </a-tabs>

        <a-tabs>
          <a-tab-pane key="1" tab="测试结果（刷新自动清空）">
            <a-table
              :columns="columns"
