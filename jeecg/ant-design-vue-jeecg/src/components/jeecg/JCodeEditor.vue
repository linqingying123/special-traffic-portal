<template>
  <div v-bind="fullScreenParentProps">
    <a-icon v-if="fullScreen" :type="iconType" class="full-screen-icon" @click="()=>fullCoder=!fullCoder"/>

    <div class="code-editor-cust full-screen-child">
      <textarea ref="textarea"></textarea>
      <span :class="{'null-tip-hidden':hasCode}" :style="nullTipStyle" class="null-tip" @click="nullTipClick">{{ placeholderShow }}</span>
      <template v-if="languageChange">
        <a-select v-model="mode" class="code-mode-select" placeholder="请选择主题" size="small" @change="changeMode">
          <a-select-option
            v-for="mode in modes"
            :key="mode.value"
            :value="mode.value">
