    ...
    <a-spin :spinning="confirmLoading">
      <a-form-model ref="form" :model="model" :rules="validatorRules">
        ...
        <a-form-model-item :labelCol="labelCol" :wrapperCol="wrapperCol" label="规则Code" prop="ruleCode">
          <a-input v-model="model.ruleCode" placeholder="请输入规则Code"/>
        </a-form-model-item>
        <a-form-model-item :labelCol="labelCol" :wrapperCol="wrapperCol" label="规则描述" prop="ruleDescription">
          <a-textarea v-model="model.ruleDescription" placeholder="请输入规则描述"/>
        </a-form-model-item>

      </a-form-model>
      <!-- 规则设计 -->
      <a-tabs v-model="tabs.activeKey">
        <a-tab-pane :key="tabs.design.key" forceRender tab="局部规则">
          <a-alert message="局部规则按照你输入的位数有序的校验。" showIcon type="info"/>
          <j-editable-table ref="designTable" :columns="tabs.design.columns" :dataSource="tabs.design.dataSource"  :maxHeight="240" dragSort rowNumber style="margin-top: 8px;">
        ...
        <a-tab-pane :key="tabs.global.key" forceRender tab="全局规则">
          <j-editable-table ref="globalTable" :columns="tabs.global.columns" :dataSource="tabs.global.dataSource" :maxHeight="240" actionButton dragSort  rowNumber>

            <template #actionButtonAfter>
              <a-alert message="全局规则可校验用户输入的所有字符；全局规则的优先级比局部规则的要高。" showIcon style="margin-bottom: 8px;" type="info"/>
            </template>
