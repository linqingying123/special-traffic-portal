<template>
  <a-modal
    ...
    <a-form-model>
      <a-form-model-item v-if="isMultiTenant" :labelCol="{span:4}" :validate-status="validate_status1" :wrapperCol="{span:20}" style="margin-bottom:10px">
        <a-tooltip placement="topLeft" >
          <template slot="title">
            <span>您有多个租户，请选择登录租户</span>
          </template>
          <a-avatar icon="gold" style="backgroundColor:#87d068" />
        </a-tooltip>

        <a-select :class="{'valid-error':validate_status1=='error'}" placeholder="请选择登录租户" style="margin-left:10px;width: 80%" @change="handleTenantChange">
      ...
      <a-form-model-item v-if="isMultiDepart" :labelCol="{span:4}" :validate-status="validate_status2" :wrapperCol="{span:20}" style="margin-bottom:10px">
        <a-tooltip placement="topLeft" >
          <template slot="title">
            <span>您有多个部门，请选择登录部门</span>
          </template>
          <a-avatar icon="gold" style="backgroundColor:rgb(104, 208, 203);" />
        </a-tooltip>

        <a-select :class="{'valid-error':validate_status2=='error'}" placeholder="请选择登录部门" style="margin-left:10px;width: 80%" @change="handleDepartChange">
          <a-icon slot="suffixIcon" type="gold" />
